...
module.exports = {
    ...
    permission: Discord.PermissionFlagsBits.BanMembers || Discord.PermissionFlagsBits.Administrator,
    dm: false,
    category : "Modération",
    options: [
        {
    ...
    async run(bot, message, args) {
        try {
            ...
            if((await message.guild.bans.fetch()).get(user.id)) return message.reply("Ce membre est déjà banni du serveur !")

            try {await user.send(`Tu as été banni du serveur ${message.guild.name} par ${message.user.tag} pour la raison suivante : \`${reason}\``)} catch
(err) {}
